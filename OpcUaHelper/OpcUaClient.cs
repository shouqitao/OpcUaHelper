...
namespace OpcUaHelper {
    ...
    public class OpcUaClient {
        ...
        public async Task ConnectServer(string serverUrl) {
            Session = await Connect(serverUrl);
        }
        ...
        private async Task<Session> Connect(string serverUrl) {
            ...
            var endpointDescription = CoreClientUtils.SelectEndpoint(serverUrl, UseSecurity);
            ...
            Session = await Session.Create(
                ...
                false,
                false,
                (string.IsNullOrEmpty(OpcUaName)) ? AppConfig.ApplicationName : OpcUaName,
                60000,
                UserIdentity,
                new string[] { });
            ...
            try {
                ...
                if (ServiceResult.IsBad(e.Status)) {
                    ...
                    if (m_reconnectHandler == null) {
                        ...
                        m_reconnectHandler.BeginReconnect(Session, ReconnectPeriod * 1000,
        ...
        public void AddNewNode(NodeId parent) {
            ...
            var nodesToAdd = new AddNodesItemCollection { node2 };
        ...
        public void AddSubscription(string key, string tag,
        ...
        public void AddSubscription(string key, string[] tags,
            Action<string, MonitoredItem, MonitoredItemNotificationEventArgs> callback) {
            var m_subscription = new Subscription(Session.DefaultSubscription) {
        ...
        /// <param name="containBound">是否包含边界</param>
        /// <returns>读取的数据列表</returns>
        public IEnumerable<T> ReadHistoryRawDataValues<T>(string tag, DateTime start, DateTime end, uint count = 1,
            bool containBound = false) {
            var m_nodeToContinue = new HistoryReadValueId() {
                NodeId = new NodeId(tag),
            };
        ...
        private void CheckReturnValue(StatusCode status) {
            if (!StatusCode.IsGood(status))
                throw new Exception($"Invalid response from the server. (Response Status: {status})");
        }
